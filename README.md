...
**The agent has arrived (Track 8, phase 1).** A third service — `agent/`, Python FastAPI + LangGraph — hosts a **read-only analyst**: chat with it on the `/agent` page and it answers questions like "which run made the most money and where is its constraint?" by reading the sim through the backend's REST API (its entire tool surface — it inherits run locks, frozen-config semantics and seed reproducibility like any other client). Phase 1 on purpose: it can read everything and change nothing. Action tools, the fork-and-compare experiment graph, and LangSmith evals are the next phases.

...
- Live throughput chart: the run's stored per-tick throughput, accumulated. The old trailing-mean smoothing is gone with the frontend engine. The series is capped at the newest 5000 ticks, so the curve carries on from what the run had already earned rather than re-basing at zero — exactly, because per-tick money and the run's frozen per-part money are two sums of the same credits.
